...
bool init()
{
    ...
    if (g_mutex)
    {
        throw "Buds detection cannot be initialized twice";
    }

    g_mutex = CreateMutex(NULL, false, NULL);

    g_lpiRescanRequests = (LONG *)_aligned_malloc(sizeof(LONG), 32);
    if (g_lpiRescanRequests)
    {
        *g_lpiRescanRequests = 0l;
...
DWORD WINAPI StartListenThread(LPVOID lpParam)
{
    auto threadCount = InterlockedIncrement(g_lpiRescanRequests);
    ...
    if (threadCount == 1)
    {
        ...
        while (threadCount)
        {
            ...
            __finally
            {
                threadCount = InterlockedDecrement(g_lpiRescanRequests);
            }
...
void ProcessNewDevices(bool flushCache)
{
    ...
    if (CXN_SUCCESS == iResult)
    {
        ...
        while (bContinueLookup)
        {
            ...
            {
                // Got one - attempt to open a socket to the required end point
